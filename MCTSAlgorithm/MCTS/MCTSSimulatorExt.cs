...
namespace SabberStoneAICompetition.MCTSAlgorithm.MCTS
{
    ...
    class MctsSimulatorExt : MctsSimulator
    {
		...
		/// </summary>
		private readonly BoardCards _oppBoardCards;
		...
		public override MctsNode Simulate(POGame game)
		{
			...
			var parent = new MctsNode(_playerId, new List<MctsNode.ScoreExt> { new MctsNode.ScoreExt(1, _scoring) }, gameCopy, null, null);
			...
			IReadOnlyList<Prediction> predicitionMap, ref Dictionary<POGame, List<MctsNode>> newSimulations)
		{
			...
			if (!(predicitionMap?.Any() ?? false)) return predictionScore;
			...
			for (int i = 0; i < predicitionMap.Count; i++)
			{
				       ...
				       && forwardGame.CurrentPlayer.Id == opponent.Id)
				{
					...
					for (int j = 0; j < solutions.Count && (forwardGame != null); j++)
					{
						...
						Dictionary<PlayerTask, POGame> dir = forwardGame.Simulate(new List<PlayerTask> { oppTask });
				       ...
				       && forwardGame.CurrentPlayer.Id == _playerId)
				{
					...
					for (int j = 0; j < solutions.Count && (forwardGame != null); j++)
					{
						...
						Dictionary<PlayerTask, POGame> dir = forwardGame.Simulate(new List<PlayerTask> { task });
		...
		private List<Prediction> GetPredictionMap(POGame simulationGame, Controller opponent)
		{
			...
			if (!(playedCards.CardEntries?.Any() ?? false)) return predictionMap;
		...
		private PlayedCards UpdateTasks(Controller opponent)
		{
			double initValue = 1;
		...
		private List<PredictedCards> PredictCards(int cardCount, PlayedCards playedCards,
			CardSet boardCards, Dictionary<CardClass, double> classPrediction)
		{
			...
			ref List<Prediction> predictionMap)
		{
				...
				&& deckCount < _predictionParameters.DeckCount; d++)
			{
					...
					i += _predictionParameters.StepWidth)
				{
					...
					predictionMap.Add(new Prediction(hand, deck));
					predictionsCount++;
				}
				deckCount++;
			}
		...
		private Controller GetOpponent(POGame game)
		{
		...
		private abstract class CardSet
		{
			...
			/// </summary>
			public List<KeyValuePair<Card, double>> CardEntries { get; }
		...
		private class Prediction
		{
			...
			public Prediction(Hand hand, Deck deck)
			{
				Hand = hand;
				Deck = deck;
			}
